...
import { View, Text, FlatList, StyleSheet, Pressable, Modal, Animated, Platform } from 'react-native';
import { SafeAreaView } from "react-native-safe-area-context";
import { useNavigation } from '@react-navigation/native';
import DateTimePicker from "@react-native-community/datetimepicker";
import { getDays } from '../controllers/days_controllers';
import { getExercises } from '../controllers/exercises_controllers';
import { createSession } from '../controllers/workout_controllers';
...
const DayItem = ({ item, index }) => {
    ...
    const toggle = () => {
        ...
        Animated.parallel([
            Animated.timing(animHeight,  { toValue, duration: 250, useNativeDriver: false }),
            Animated.timing(animRotate,  { toValue, duration: 250, useNativeDriver: true }),
    ...
    const opacity   = animOpacity.interpolate({ inputRange: [0, 1], outputRange: [0, 1] });

    useEffect(() => {
        fetchExercisesForDay(item.day_id);
    }, []);
                ...
                <View style={styles.dropdownInner}>
                    ...
                    {exercises.length > 0 ? (
                        exercises.map((exercise) => (
                            <Text key={exercise.id} style={styles.dropdownText}>
                                •  {exercise.ex_name}  -  {exercise.sets} sets  -  {exercise.reps} reps
                            </Text>
                        ))
                    ) : (
...
const Days = (props) => {
    ...
    const onChangeDate = (event, selectedDate) => {
        ...
        if (activePicker === 'start') {
            ...
            if (endDate < pickedDate) setEndDate(pickedDate);
    ...
    const handleStartWorkout = async () => {
        try {
            const data = await createSession(workoutID, startDate.toISOString(), endDate.toISOString());
    ...
    return (
        <SafeAreaView style={[g.container, { alignItems: 'stretch', paddingHorizontal: 0, justifyContent: 'center' }]}>
            <FlatList
                ...
                renderItem={({ item, index }) => <DayItem item={item} index={index} />}
                ...
                <View style={g.modalOverlay}>
                    <View style={g.modalContainer}>
                        ...
                        <View style={styles.InputsHolder}>
                            <View style={styles.dateBlock}>
                                ...
                                <Pressable
                                    ...
                                    onPress={() => { setActivePicker('start'); setShowPicker(true); }}
                                >
                            ...
                            <View style={styles.dateBlock}>
                                ...
                                <Pressable
                                    style={styles.dateButton}
                                    onPress={() => { setActivePicker('end'); setShowPicker(true); }}
                                >
                        ...
                        <View style={styles.modalButtonsHolder}>
                            <Pressable style={[g.buttonOutline, styles.modalButton]} onPress={() => setIsModalOpen(false)}>
                                <Text style={g.buttonOutlineText}>Cancel</Text>
                            </Pressable>
                            <Pressable style={[g.button, styles.modalButton]} onPress={handleStartWorkout}>
...
const styles = StyleSheet.create({
    arrow:             { color: '#ff6600', fontSize: 24, marginRight: 15 },
    dropdown:          { backgroundColor: '#1a1a1a', borderWidth: 1, borderTopWidth: 0, borderColor: '#333', borderRadius: 10, marginBottom: 8 },
    dropdownInner:     { padding: 14, gap: 8 },
    ...
    buttonContainer:   { gap: 8, marginBottom: 8 },
    InputsHolder:      { width: '100%', marginTop: 8, paddingHorizontal: 20, paddingBottom: 24, gap: 10 },
    dateBlock:         { width: '100%', marginBottom: 6 },
    dateLabel:         { color: '#fff', fontSize: 16, fontWeight: '600', marginBottom: 8 },
    dateButton:        { paddingVertical: 10, paddingHorizontal: 14, borderRadius: 8, borderWidth: 1, borderColor: '#444', backgroundColor: '#1a1a1a' },
    dateButtonText:    { color: '#fff', fontSize: 14 },
    modalButtonsHolder:{ flexDirection: 'column', width: '100%', marginTop: 8, paddingHorizontal: 20, paddingBottom: 20 },
    modalButton:       { width: '100%', marginHorizontal: 0 },
});
